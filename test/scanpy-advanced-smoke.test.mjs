// test/scanpy-advanced-smoke.test.mjs — smoke tests for BLOCK-11 PHASE 1 SUB-1.1
//
// 验证（不依赖 deseq2/R runtime）：
...
//   6. demo h5ad 落盘且能读 (如未生成，承认 dev Electron 边界内的 scanpy runtime 依赖)
//
// 注：本测试不调 ./scripts/scanpy-advanced.py 端到端（那是 dev Electron 边界内
// 单元测试范围——end-to-end 已由 BLOCK-11 协调 worker 在 audit 阶段跑过）；
// 这里只验证 plugin 8 件套的合规与 schema 校验。
//
// 运行：
//   node --test test/scanpy-advanced-smoke.test.mjs
...
const pluginDir = resolve(repoRoot, 'plugins/scanpy-advanced');
const require = createRequire(import.meta.url);

test('plugin.json 存在 + 顶层 manifest.json 的 scanpy-advanced 条目通过 schema 校验', () => {
  ...
  assert.equal(plugin.id, 'scanpy-advanced');
  ...
  assert.ok(
    manifest.plugins['scanpy-advanced'],
    'manifest.json scanpy-advanced entry missing after append'
  );
  const entry = manifest.plugins['scanpy-advanced'];
  ...
  assert.equal(entry.artifacts['macos-arm64'].url, 'built-in://scanpy-advanced');
});

test('environment.json 存在且 packages.python 至少含 scanpy + anndata', () => {
...
test('manifest.entry.json 存在且含 entryPoint 指向 scripts/scanpy-advanced.py', () => {
  ...
  assert.equal(e.pluginId, 'scanpy-advanced', 'pluginId mismatch');
  assert.equal(
    e.entryPoint,
    'scripts/scanpy-advanced.py',
    'entryPoint should point to scripts/scanpy-advanced.py'
  );
...
test('install.json 存在且声明 no-install-required', () => {
  ...
  assert.equal(i.pluginId, 'scanpy-advanced');
  // PHASE 1 SUB-1.1 边界内 runtime 不扩展；install.steps 应为空数组
...
test('scripts/scanpy-advanced.py 存在且为合法 Python 入口', () => {
  const path = resolve(pluginDir, 'scripts/scanpy-advanced.py');
  assert.ok(existsSync(path), 'scripts/scanpy-advanced.py missing');
  const stats = statSync(path);
  assert.ok(stats.size > 1000, `scripts/scanpy-advanced.py too small (${stats.size} bytes)`);
...
test('tool-definition.json 含 8 个 outputs 且 ipcChannel + failureModes 完整', () => {
  ...
  assert.equal(td.id, 'scanpy-advanced');
  ...
  const expectedOutputs = [
    'preprocessed', 'marker_genes', 'qc_summary',
    'umap', 'umap_clustered', 'hvg_plot',
    'report', 'manifest',
  ];
  ...
  assert.ok(td.ipcChannel === 'tools:invoke/scanpy-advanced', 'ipcChannel mismatch');
...
test('acceptance.json 4 段 fixture (valid + missing-inputs + runtime-not-ready + empty-result)', () => {
  ...
  assert.equal(a.pluginId, 'scanpy-advanced');
...
test('顶层 manifest.json 已包含 scanpy-advanced 条目', () => {
  const path = resolve(repoRoot, 'plugins/manifest.json');
  const manifest = JSON.parse(readFileSync(path, 'utf8'));
  assert.ok(manifest.plugins['scanpy-advanced'], 'manifest.json scanpy-advanced entry missing');
  assert.equal(manifest.plugins['scanpy-advanced'].id, 'scanpy-advanced');
  assert.equal(manifest.plugins['scanpy-advanced'].language, 'python');
  assert.equal(manifest.plugins['scanpy-advanced'].kind, 'plugin');
});

test('scanpy-advanced plugin 目录结构完整（8 件套文件全部存在）', () => {
  const required = [
    'plugin.json',
    'environment.json',
    ...
    'install.json',
    'README.md',
    'scanpy-advanced-report-template.html',
    'scripts/scanpy-advanced.py',
...
test('scanpy-advanced plugin 目录无未提交孤儿文件', () => {
